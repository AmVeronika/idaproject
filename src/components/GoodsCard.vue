<template>
  <div :class="$style['goods-card']">
    <div :class="$style['good-card']" v-for="good in getGoodsList" :key="good">
      <img
        :src="good.urlImg"
      ...
      <div :class="$style['good-card__wrapp']">
        <h3 :class="['fs-20', $style['good-card__title']]">
          {{ good.name }}
        </h3>
        <p :class="['fs-16', $style['good-card__description']]">
          {{ good.description }}
        </p>
        <p :class="[$style['good-card__price'], 'fs-24']">
          {{ good.price }}<span class="$style['good-card__price-currency']"> руб.</span>
        </p>
      </div>
      <svg :class="$style['good-card__cart']" width="40" height="40" viewBox="0 0 40 40" fill="none"
